...
def extract_images_from_html(html_content, article_url):
    ...
    for img in content_div.find_all('img'):
        ...
        if 'mmbiz' in src.lower():
            # 对于gif格式，检查尺寸和URL特征，小图标通常不是文章图片
            if 'wx_fmt=gif' in src.lower():
                ...
                try:
                    ...
                    pass
                # 检查URL中是否有明显的分隔符特征（重复的URL通常是分隔符）
                # 如果这个URL在之前的图片中出现过，可能是分隔符
                if any(existing_img['url'] == src for existing_img in images):
                    continue
            is_valid = True
...
def process_article(post_file):
    ...
    elif len(images) <= existing_count and missing_captions:
        print(f"  图片数量足够，但有 {len(missing_captions)} 个图片说明缺失图片，需要处理")
    else:
        print(f"  图片数量足够，无需添加")
        return False
    
    # 需要插入的图片数量
    # 如果文章中没有图片，插入所有图片；如果有缺失的图片说明，至少插入缺失的数量
    if existing_count == 0:
        need_insert = len(images)
    elif missing_captions:
        # 如果有缺失的图片说明，至少为它们插入图片
        need_insert = max(len(missing_captions), len(images) - existing_count)
    else:
        need_insert = max(0, len(images) - existing_count)
    
    # 如果有缺失的图片说明，优先为它们插入图片
    if missing_captions and need_insert > 0:
        print(f"  有 {len(missing_captions)} 个图片说明缺失图片")
        lines = md_content.split('\n')
        inserted = 0
        
        # 从后往前插入，避免位置偏移
        for i in range(len(missing_captions) - 1, -1, -1):
            if inserted >= need_insert:
                break
    ...
    # 如果文章很短（少于10行），可能不需要图片
    if not missing_captions and len(content_lines) < 10:
        print(f"  文章内容很短（{len(content_lines)} 行），且无图片说明，跳过")
        return False
    
    # 如果文章有内容但没有图片说明，在文章合适位置插入图片
    if need_insert > 0 and len(content_lines) >= 10:
        print(f"  文章较长但无图片说明，在合适位置插入图片")
        lines = md_content.split('\n')
        ...
        # 在文章中间位置插入图片（大约1/3处）
        insert_pos = content_start + len(content_lines) // 3
        
        inserted = 0
        for i in range(min(need_insert, len(images) - existing_count)):
            img_idx = existing_count + i
            if img_idx < len(images):
                img = images[img_idx]
                img_url = img['url']
